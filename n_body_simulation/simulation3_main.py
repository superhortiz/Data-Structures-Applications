...
def main() -> None:
    ...
    for i in range(n):
        bodies.append(Body([WIDTH // 2, HEIGHT // 2],
            [np.random.randint(500,600)*np.cos(2*np.pi*i/n), np.random.randint(500,600)*np.sin(2*np.pi*i/n)],
            mass*20, np.random.randint(50,255,3), 3))
